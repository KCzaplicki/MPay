...
{
    private static readonly string[] IgnoredPlaceholderValues = { "PropertyName", "PropertyValue", "ComparisonProperty", "TotalLength" };
    ...
    private static ValidationError CreateValidationError(ValidationFailure validationFailure) 
        => new(MapErrorCode(validationFailure), MapErrorParameters(validationFailure), validationFailure.ErrorMessage);

    private static string MapPropertyName(IGrouping<string, ValidationFailure> grouping) 
        => grouping.Key.Camelize();
    ...
    private static string MapErrorCode(ValidationFailure e) 
        => e.ErrorCode.Underscore().Replace("_validator", "_error").ToUpperInvariant();
}