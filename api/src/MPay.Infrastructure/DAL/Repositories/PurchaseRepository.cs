...
internal class PurchaseRepository : IPurchaseRepository
{
    ...
    {
        return _purchases.Include(p => p.Payments).FirstOrDefaultAsync(p => p.Id == id);
    }
    ...
    {
        return _purchases.Include(p => p.Payments).Where(p => p.Status == PurchaseStatus.Pending).ToListAsync();
    }
}