...
public class BackgroundDispatcherService : BackgroundService
{
    ...
    public BackgroundDispatcherService(IServiceProvider serviceProvider, IEventChannel eventChannel, ILogger<BackgroundDispatcherService> logger)
    {
        ...
        await foreach(var @event in _eventChannel.Reader.ReadAllAsync(stoppingToken))
        {
            try
            {
                ...
                foreach (var eventHandler in eventHandlers)
                {
                    ...
                    _logger.LogInformation($"Event '{@event.GetType().Name}' is being handled by '{eventHandler.GetType().Name}'. Event payload: '{@event}'.");
                }
